...
* **MaquinaEstados**: Definição da Classe abstrata MaquinaEstados (Será a classe pai para a implementação de scripts que utilizarão uma Máquina de Estados);
...
## Como usar:
A intenção do pacote é possibilitar o uso de Máquina de Estados para diferentes contextos (Player com diferentes estados, Inimigos com comportamentos complexos etc.).
Na pasta "Exemplos" há um exemplo de implementação de Máquina de Estados, sendo que para criar um nova implementação e criar seus estados deve-se seguir esses passos (Recomendado acessar o exemplo para melhor entendimento do processo):

1. Criar uma nova classe com base na classe pai "MaquinaEstados":

`public class NomeDaClasse : MaquinaEstados<NomeDaClasse>`

2. Implementar as características especificas da nova classe.
...
6. Dentro do estado, inicialize o estado com base no contexto atual da Máquina de Estados:

`public InimigoIdle(InimigoStateMachine contextoAtual) : base(contextoAtual) { }`

7. Ao criar novos estados, lembre sempre de defini-los na classe de implementação da Máquina de estados e inicializar-los em "Awake()".

8. Na implementação da nova classe filha use a função "TrocaEstado" em "Start" para definir o estado inicial:

`TrocaEstado(EstadoIdle);`

Obs.: Consulte os scripts e cena exemplos caso o funcionamento não esteja claro.
